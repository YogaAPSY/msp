        ...
        <div class="panel-heading">
          <div class="row">
            ...
            <div style="font-size:2.5em"> <?php 
              $sql = mysqli_query($koneksi , "SELECT * FROM comments");
...
<?php
  ...
  $articles = "SELECT * from comments LIMIT $start , $perpage";
  $results2 = mysqli_query($koneksi, $articles);

  $results = mysqli_query($koneksi , "SELECT * FROM comments");
    ...
    <table class="table table-striped">
      ...
      echo'
        <tr>
          ...
          <td>'.$rows['email'].'</td>
          <td>'.$rows['subject'].'</td>
          <td>'.substr($rows['comment'], 0 , 100).' ... </td>
        </tr>
        ';
        $no++;
      }
